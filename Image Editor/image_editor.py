...
control_col = sg.Column([
    [sg.Frame('Blur', layout=[[sg.Slider(range=(0, 10),
                                         orientation='h', key='-BLUR-')]])],
    [sg.Frame('Contrast', layout=[[sg.Slider(range=(0, 10),
    ...
    [sg.Button('Save Image', key='-SAVE-', enable_events=True)]
])
image_col = sg.Column([[sg.Image(image_path, key='-IMAGE-', enable_events=True)]])
...
while True:
    ...
    update_image(original, values['-BLUR-'], values['-CONTRAST-'], values['-EMBOSS-'],
                 values['-CONTOUR-'], values['-FLIPX-'], values['-FLIPY-'])
