...
def convert_pos_to_pixel(cell):
    topl = cell[0] * CELL_SIZE, cell[1] * CELL_SIZE
    botr = topl[0] + CELL_SIZE, topl[1] + CELL_SIZE
    return topl, botr
...
while True:
    ...
    if time_since_start >= 0.3:
        ...
        # draw apple
        topl, botr = convert_pos_to_pixel(apple_pos)
        field.DrawRectangle(topl, botr, 'red')
        # draw snake
        for index, part in enumerate(snake_body):
            topl, botr = convert_pos_to_pixel(part)
            color = 'yellow' if index == 0 else 'green'
            field.DrawRectangle(topl, botr, color)
