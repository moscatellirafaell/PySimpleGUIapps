...
def get_weather_data(location):
    url = f"https://www.google.com/search?q=temperatura+{location.replace(' ', '')}"
    session = requests.Session()
    session.headers['User-Agent'] = "Mozilla/5.0 (Windows NT 10.0; x64) AppleWebKit/537.36 (KHTML, like Gecko) " \
...
info_col = sg.Column([
    [sg.Text('', key='-LOCATION-', font='Calibri 30', background_color='#FF0000', text_color='#FFFFFF',
             pad=0, visible=False)],
